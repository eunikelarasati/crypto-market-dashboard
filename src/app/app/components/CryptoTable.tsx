...
//       // cek perubahan sebelum update (hindari re-render kalau sama)
...
export default function CryptoTable({ initialData }: { initialData: Coin[] }) {
  ...
  // 🔄 Realtime Refresh (10 detik sekali, hanya untuk daftar awal)
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const res = await fetch(
  ...
  useEffect(() => {
    ...
    const delayDebounce = setTimeout(async () => {
      ...
      try {
        ...
        // Step 2: ambil detail market untuk coin hasil search
